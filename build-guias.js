...
function buildLocale(lc) {
  ...
  // inyectar link de Guías en Recursos (solo si el footer extraído no lo tiene ya:
  // las páginas fuente ahora suelen incluirlo, y sin este chequeo se duplicaba)
  const guiaHref = path.basename(L.indexSlug);
  const guiaLi = `          <li><a href="${guiaHref}">${L.guiaWord}</a></li>\n`;
  const bottom = bp.bottom.includes(`href="${guiaHref}"`)
    ? bp.bottom
    : bp.bottom.replace(bp.resourcesAnchor, bp.resourcesAnchor + guiaLi);
